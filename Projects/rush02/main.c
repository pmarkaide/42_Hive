...
int **ft_split_nbr(unsigned int nbr, int **chunks);
...
int main(int argc, char **argv)
{
	...
	v = ft_count_valid_chunks(nbr);

	if(v == 4)
	{
		ft_print_1B(chunks[0], dic);
	}
	
	if(v >= 3)
	{
		ft_print_1M(chunks[1], dic);
	}
	
	if (v >= 2)
	{
		ft_print_1K(chunks[2], dic);
	}
	
	ft_print_100(chunks[3], dic);
